-- ════════════════════════════════════════════════════════════════
-- 0042 — Agrega POS como método de pago visible en Caja. Aditivo:
-- ensancha la lista de métodos válidos dentro de registrar_movimiento_caja
-- (no hay CHECK de tabla para esto, se valida en la función) — nunca se
-- angosta. "tarjeta" se mantiene en la lista solo por compatibilidad con
-- cualquier movimiento que ya lo tenga guardado; el frontend deja de
-- ofrecerlo como opción nueva y ofrece "POS" en su lugar.
-- ════════════════════════════════════════════════════════════════
...
-- ── Verificación ──
-- select prosrc ilike '%''pos''%' from pg_proc where proname='registrar_movimiento_caja'; -- true

-- ── Rollback ──
-- Volver a aplicar el CREATE OR REPLACE de 0039 tal cual estaba (sin 'pos').
