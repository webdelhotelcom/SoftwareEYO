-- ════════════════════════════════════════════════════════════════
-- 0039 — Movimientos manuales de Caja (sin reserva) para Modo
-- Propietario: ensancha el CHECK de payment_operations.tipo (nunca lo
-- angosta — sigue aceptando todo lo que ya aceptaba) y reemplaza
-- registrar_movimiento_caja (0034) para que:
--   1) el signo de un egreso-manual lo decida el SERVIDOR, no el
--      navegador (el llamador siempre manda un importe positivo);
--   2) metodo_pago se valide contra una lista cerrada, así
--      calcular_expected_cash() (0040) nunca depende de texto libre.
-- ════════════════════════════════════════════════════════════════

alter table public.payment_operations drop constraint if exists payment_operations_tipo_check;
alter table public.payment_operations add constraint payment_operations_tipo_check
  check (tipo in ('sena','pago-parcial','pago-final','cobro-checkin','cobro-checkout',
                   'anulacion','devolucion','ingreso-manual','egreso-manual'));
...
begin
  ...
  if p_metodo is not null and p_metodo not in ('efectivo','transferencia','tarjeta','otro') then
...
-- ── Verificación ──
-- select conname from pg_constraint where conname='payment_operations_tipo_check';
-- select prosrc ilike '%egreso-manual%' from pg_proc where proname='registrar_movimiento_caja'; -- true

-- ── Rollback ──
-- No hay forma automática de "volver" a la versión anterior de la función
-- una vez reemplazada — si hiciera falta, se vuelve a aplicar el CREATE OR
-- REPLACE de 0034_rpc_flujo_propietario.sql tal cual estaba.
-- alter table public.payment_operations drop constraint if exists payment_operations_tipo_check;
-- alter table public.payment_operations add constraint payment_operations_tipo_check
--   check (tipo in ('sena','pago-parcial','pago-final','cobro-checkin','cobro-checkout','anulacion','devolucion'));
